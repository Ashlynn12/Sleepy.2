...
function format(d) {
  return (splice(d.getFullYear().toString(), 0, 2, "") + "-" + pad(d.getMonth() + 1, 2) + "-" + pad(d.getDate(), 2) + " " + pad(d.getHours(), 2) + ":" + pad(d.getMinutes(), 2) + ":" + pad(d.getSeconds(), 2));
}
  ...
  static log(content, options = {}) {
    ...
    if (typeof content !== "string") {
      if (typeof content === "object" && Object.prototype.toString.call(content).match(/\[object (.+)]/)[1] === "Error") {
        ...
        content = require("util").inspect(content, { depth: 5 });
      }
    }
