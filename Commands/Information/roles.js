...
module.exports = class extends Command {
    ...
    async run(message, args) {
        ...
        const membersWithRole = message.guild.members.cache.filter(member => member.roles.cache.find(r => r.name === role.name)).map(member => member.user.tag),
            roleEmbed = new MessageEmbed()
                .setColor("RANDOM")
                .setThumbnail(message.guild.iconURL())
                .setTitle(`Users With The ${role.name} Role! [${membersWithRole.length}]`)
                .setDescription(membersWithRole.join("\n"));
        message.channel.send(roleEmbed).catch(async () => {
